<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?color=%2336BCF7&center=true&vCenter=true&lines=H+A+P+P+Y+++N+E+W+++Y+Y+E+A+R++2026" />
</p>

### THOSE WHOSE BACKUP DOESN'T WORK PLEASE REINSTALL, SORRY FOR THE INCONVENIENCE AS I'VE BEEN A BIT BUSY

### LifeTime Free Script V1
## Scroll down and read everything

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?color=%2336BCF7&center=true&vCenter=true&lines=S+C+R+I+P+T++A+R+Y+A++B+L+I+T+A+R" />
</p>

```
apt install -y && apt update -y && apt upgrade -y && wget -q https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/setup.sh && chmod +x setup.sh && ./setup.sh
```

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/scp.png)

## FOR HAPROXY, DON'T INSTALL UBUNTU 22 OR ABOVE — DOWNGRADE TO DEBIAN 10/11 INSTEAD. IF YOU WANT HELP UPGRADING TO UBUNTU 20.04, PLEASE WHATSAPP ME AT +6281450330727

FREE HAPROXY SCRIPT, NO IP PERMISSION NEEDED DEAR... DEBIAN 10/11 / UBUNTU 20.18, 20.04 — JUST INSTALL DIRECTLY!!!
RECOMMENDED TO USE YOUR OWN DOMAIN.

Multiport
SSH WS/UDP/SSL/SlowDNS/Vray/Shadowsocks/
NoobzVPN/Telegram Bot

If anything errors, you can Fixcert, Restart Service. If Dropbear errors, choose option 21 then option 7 & Nginx Haproxy Fix is option 8 / 9

### Donations welcome, whatever you're comfortable with 🤣

Lightweight version, no UDP/SlowDNS/NoobzVPN, no limit V2

```
apt install -y && apt update -y && apt upgrade -y && wget -q https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/main.sh && chmod +x main.sh && ./main.sh
```

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/scu.png)

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/scc.png)

### How to Fix Dropbear Haproxy Nginx

https://youtu.be/5oyRchPMzJc?si=5n9sxwrbrn2y4URd

Premium SSH accounts available 10k ISP Indo & SGDO

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/gb.png)

Lightweight Version Stunnel4 Script Here
https://github.com/Arya-Blitar22/auto

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/law.png)

### Pointing domain to wildcard

https://youtu.be/WCFd59YzcHY

## ALL OS SCRIPT STUNNEL4

STUNNEL4 > 444,445,447,777

SSH-WS > 80,8880

SSH-WS-TLS > 443,8443

VRAY-VMESS-VLESS WSTLS 443,8443 WS 80,8880

TROJAN WSTLS 443,8443

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/obat.png)

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/ubuntu.png)

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/soca.png)

## OS Install Link Ubuntu 22.04 / 24.04 / 25.04
```
apt install -y && apt update -y && apt upgrade -y && wget -q https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/setup7.sh && chmod +x setup7.sh && ./setup7.sh
```

## Your Haproxy

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/koplak.png)

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/koplak2.png)

![logo](https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/api.png)

### All OS Haproxy Script

```
wget --no-check-certificate https://raw.githubusercontent.com/Arya-Blitar22/st-pusat/main/stupul.sh && chmod +x stupul.sh && ./stupul.sh
```

## For all OS, check the latest video

https://youtu.be/IDM4zs_E4GY

### CONTACT
<a href="https://t.me/AryaBlitar" target="_blank"><img src="https://img.shields.io/static/v1?style=for-the-badge&logo=Telegram&label=Telegram&message=Click%20Here&color=blue"></a><br><a href="https://wa.me/6281450330727" target="_blank"><img src="https://img.shields.io/static/v1?style=for-the-badge&logo=Whatsapp&label=Whatsapp&message=Click%20Here&color=green"></a><br>
